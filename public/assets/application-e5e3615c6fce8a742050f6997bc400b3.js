...
(function() {
  var codeLatLng, geocoder, handler, listenerMapClick, marker, panorama, placeMarker, placeNewMarker, showNewPosition, showNewPositionAddress;
  ...
  this.buildMap = function(markers) {
    ...
    return panorama.setPov({
      heading: 265,
      pitch: 0
    });
  };

  this.buildEmptyMap = function() {
    handler = Gmaps.build('Google');
  ...
  this.buildEditMap = function(markers, lat, lng) {
    var latLng;
    handler = Gmaps.build('Google', {
      markers: {
        clusterer: {
    ...
    handler.buildMap({
      ...
      return markers = handler.addMarkers(markers);
    });
    return placeNewMarker(latLng);
  };

  this.buildNewMap = function(markers) {
    handler = Gmaps.build('Google', {
      markers: {
        clusterer: {
          gridSize: 20,
    ...
    return handler.buildMap({
      ...
      markers = handler.addMarkers(markers);
      return listenerMapClick = google.maps.event.addListenerOnce(handler.getMap(), "click", function(event) {
        return placeNewMarker(event.latLng);
      });
    });
  };
  ...
  placeNewMarker = function(location) {
    ...
    google.maps.event.addListener(marker, 'drag', function(event) {
      return showNewPosition(event.latLng);
    });
    google.maps.event.addListener(marker, 'dragend', function(event) {
      return showNewPositionAddress(event.latLng);
    });
    return codeLatLng(location);
  };
  ...
  showNewPositionAddress = function(location) {
    handler.getMap().panTo(location);
    return codeLatLng(location);
  };
  ...
  showNewPosition = function(location) {
    document.getElementById("outdoor_latitude").value = location.lat();
    return document.getElementById("outdoor_longitude").value = location.lng();
  };
  ...
  codeLatLng = function(location) {
    ...
    return geocoder.geocode({
      ...
      if (status === google.maps.GeocoderStatus.OK) {
        if (results[1]) {
          return document.getElementById("outdoor_address").value = results[1].formatted_address;
        }
      } else {
        return alert("ERRO Geocoder: " + status);
      }
    });
  };
  ...
  this.codeAddress = function() {
    ...
    return geocoder.geocode({
      ...
      if (status === google.maps.GeocoderStatus.OK) {
        ...
        return placeNewMarker(results[0].geometry.location);
      } else {
        return alert("ERRO Geocoder: " + status);
      }
    });
  ...
  this.toggleStreetView = function(lat, lng) {
    var position, toggle;
    position = new google.maps.LatLng(lat, lng);
    panorama.setPosition(position);
    toggle = panorama.getVisible();
    if (toggle === false) {
      return panorama.setVisible(true);
    } else {
